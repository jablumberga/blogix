  ...
  if (request.method === "POST") {
    ...
    if (sessionUser.role === "driver") {
      ...
      if (useRelational) {
        ...
        // Exclude admin-created categories — driverPay (generated by syncAll) and nominaTotalOverride
        // must never be wiped by a driver save even if the driver's local state doesn't have them yet
        const expDeleteUrl = expIds.length > 0
          ? `${SUPABASE_URL}/rest/v1/bl_expenses?rec->>driverId=eq.${driverId}&rec->>category=neq.nominaTotalOverride&rec->>category=neq.driverPay&id=not.in.(${expIds.join(",")})`
          : `${SUPABASE_URL}/rest/v1/bl_expenses?rec->>driverId=eq.${driverId}&rec->>category=neq.nominaTotalOverride&rec->>category=neq.driverPay`;
