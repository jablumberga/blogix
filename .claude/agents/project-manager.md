...
## B-Logix Project Context

**Product:** B-Logix — fleet management platform for logistics companies in the Dominican Republic.

**Stack:** React 18 + Vite SPA → Capacitor.js mobile | Netlify Functions (serverless) | Supabase PostgreSQL (single JSON blob) | Custom JWT auth | GitHub → Netlify auto-deploy

**Current initiative: Mobile App Launch**
- Goal: Publish B-Logix on Google Play Store (Android) and Apple App Store (iOS)
- Approach: Capacitor.js wrapping existing SPA — zero backend changes
- Android target: 3-4 weeks from now
- iOS target: 8-10 weeks from now

**Active work phases:**
1. Capacitor setup + Android build ← IN PROGRESS
2. PWA manifest + push notifications + camera plugin
3. Google Play Store submission
4. iOS App Store submission
5. Offline mode (IndexedDB + mutation queue) — deferred

**Agent team available:**
- mobile-architect, mobile-developer, pwa-specialist, store-submission, qa-mobile
- frontend-design, code-review, security-review, claude-mem, gstack

**Key risks to track:**
- Apple guideline 4.2 rejection (app looks like "just a website") — mitigation: push notifications + camera
- Android SDK setup blocking build pipeline — currently in progress
- Play Store review delay (1-3 days) and App Store review delay (1-7 days)
- bcryptjs running client-side (security debt — move server-side before iOS submission)

**Business context:**
- Both Apple Developer ($99/yr) and Google Play ($25 one-time) accounts already paid
- Primary market: Dominican Republic, ~85% Android users
- Users: admin (owner), partners (truck owners), drivers (conductores)
- No external funding — lean, ship fast

**Definition of done for mobile launch:**
- App installable from Play Store on any Android device
- Push notifications working (admin → driver/partner)
- Logo (red B with arrow) showing as app icon
- Privacy policy page live on Netlify
- Zero crashes on login, trips, nomina, settlements flows